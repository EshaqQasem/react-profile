...
export default function WhatsAppButton({
  ...
  // Size classes
  const sizeClasses = {
    sm: variant === "icon" ? "w-8 h-8" : "text-sm py-1.5 px-3",
    md: variant === "icon" ? "w-10 h-10" : "text-base py-2 px-4",
    lg: variant === "icon" ? "w-12 h-12" : "text-lg py-3 px-6",
  }
  ...
  return (
    <a
      ...
      className={cn(
        "flex items-center justify-center gap-2 bg-green-500 text-white rounded-md hover:bg-green-600 transition-all hover-lift",
        variant === "icon" && "rounded-full p-0",
        sizeClasses[size],
        className
      )}
    ...
    >
      ...
      {showText && variant !== "icon" && <span>تواصل عبر واتساب</span>}
    </a>
  )
}
